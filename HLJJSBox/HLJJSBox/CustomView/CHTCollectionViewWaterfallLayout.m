...
@property (nonatomic, strong) NSMutableDictionary<NSNumber *, UIView *> *sectionBackgroundViewMap;
...
- (void)commonInit {
    ...
    self.sectionBackgroundViewMap = [NSMutableDictionary dictionary];
}

- (id)init {
    if (self = [super init]) {
        self.supportSectionColor = YES;
        [self commonInit];
    }
    return self;
}
...
- (void)prepareLayout {
    ...
    for (NSInteger section = 0; section < numberOfSections; ++section) {
        ...
        if (self.supportSectionBackgroundView) {
            NSAssert(self.itemRenderDirection == CHTCollectionViewWaterfallLayoutItemRenderDirectionShortestFirst, @"背景视图未处理其他情况，请自行测试");
            if ([self.sectionBackgroundViewMap.allKeys containsObject:@(section)]) {
                UIView *backgroundView = self.sectionBackgroundViewMap[@(section)];
                [backgroundView removeFromSuperview];
                [self.sectionBackgroundViewMap removeObjectForKey:@(section)];
            }
            if ([self.delegate respondsToSelector:@selector(collectionView:layout:backgroundViewForSectionAt:)]) {
                UIView *backgrondView = [self.delegate collectionView:self.collectionView layout:self backgroundViewForSectionAt:section];
                if (backgrondView) {
                    backgrondView.frame = CGRectMake(0, top, backgrondView.frame.size.width, backgrondView.frame.size.height);
                    backgrondView.layer.zPosition = -1;
                    [self.collectionView addSubview:backgrondView];
                    [self.sectionBackgroundViewMap setObject:backgrondView forKey:@(section)];
                }
            }
        }
